...
@section('title', 'Reservar Cita Premium | Miradent')
        ...
        <div class="booking-card fade-up" id="bookingForm">
            ...
            <div id="panelNo" class="hidden mt-10" style="border-top: 2px dashed rgba(0, 168, 132, 0.2); margin-top: 32px; padding-top: 32px;">
                <div class="yape-card" style="max-width: 500px; margin: 0 auto; box-shadow: 0 20px 40px rgba(0,0,0,0.05); text-align: center;">
                    ...
                    @if($yapeQrUrl)
                    <img src="{{ $yapeQrUrl }}" alt="Yape QR" style="margin: 0 auto 16px auto; width: 200px; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; display: block;">
                    @else
                    ...
                    <div style="font-size: 1rem; color: var(--jade-800); background: var(--jade-50); padding: 12px 24px; border-radius: 12px; display: inline-block; margin-top: 8px;">
                        Titular Yape: <span style="font-weight: 800; color: var(--jade-950); margin-left: 4px;">{{ $yapePhone ?: $doctora->phone }}</span>
                    </div>
                </div>
...
<script>
    ...
    function openWA(msg) {
        const phone = '{{ $doctora->clean_phone }}';
        if (!phone) {
    ...
    async function validarReferidoDB() {
        ...
        try {
            const response = await window.axios.post('{{ route("public.validar-referido") }}', {
