...
<head>
    ...
    <title>@yield('title') - Miradent</title>
...
<body>
    ...
    <div class="giant-bg-text">@yield('code')</div>

    <!-- Contenedor Principal (Tarjeta Glassmorphism) -->
    <div class="error-container">
        
        <!-- Icono SVG Inline o Imagen Personalizada -->
        <div class="error-icon-wrapper {{ View::hasSection('image-url') ? 'error-image-mode' : '' }}">
            @hasSection('image-url')
                <img src="@yield('image-url')" alt="Error Image" class="error-custom-image">
            @else
                @yield('icon-svg')
            @endif
        </div>
