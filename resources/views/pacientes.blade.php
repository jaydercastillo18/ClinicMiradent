...
<div class="container-fluid p-0">
    ...
    <div class="row align-items-center mb-4 pb-2">
        ...
        <div class="col-12 col-md-4 text-md-end">
            <div class="col-12 col-md-4 text-md-end">
                <a href="{{ route('pacientes.create') }}" class="btn btn-primary d-inline-flex align-items-center gap-2 shadow-sm btn-create-paciente" style="background: var(--primary); border: none; font-weight: 600; padding: 10px 20px; border-radius: 10px; font-size: 0.9rem;">
        ...
        <div class="mb-4">
            <form action="{{ route('pacientes.index') }}" method="GET">
                <div class="search-pill">
                    ...
                    <input type="text" name="search" placeholder="Escribe un nombre, DNI o teléfono..." value="{{ request('search') }}">
                    <button type="submit">Buscar</button>
                </div>
                @if(request('search'))
                <div class="mt-2 ms-2">
                    <a href="{{ route('pacientes.index') }}" class="text-danger d-inline-flex align-items-center gap-1" style="font-size: 0.8rem; font-weight: 500; text-decoration: none;">
        ...
        <div class="striking-card mb-4">
            ...
            <div class="text-center py-5 px-3">
                ...
                <p class="text-muted mb-4" style="font-size: 0.95rem;">No encontramos a nadie llamado "<strong>{{ request('search') }}</strong>"</p>
                <a href="{{ route('pacientes.index') }}" class="btn btn-light btn-sm px-3" style="border-radius: 8px; font-weight: 500;">Volver a la lista</a>
                @else
                ...
                <a href="{{ route('pacientes.create') }}" class="btn btn-primary btn-sm px-3" style="border-radius: 8px; font-weight: 500;">
                        ...
                        <tr id="paciente-row-{{ $paciente->id }}">
                            <td class="ps-4">
                                <div class="d-flex align-items-center gap-3">
                                    <div class="avatar-modern {{ $colorClass }}">
                                        {{ strtoupper(substr($paciente->nombre, 0, 1) . substr($paciente->apellido, 0, 1)) }}
                                    </div>
                                    <div>
                                        <h6 class="mb-0" style="font-weight: 700; color: #0f172a; font-size: 0.9rem;">{{ $paciente->nombre }} {{ $paciente->apellido }}</h6>
                                        <span class="text-muted" style="font-size: 0.75rem;">
                                            Reg: {{ $paciente->created_at->format('d/m/Y') }}
                                        </span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                ...
                                <span style="font-weight: 500; color: #334155; font-size: 0.85rem;">{{ \Carbon\Carbon::parse($paciente->fecha_nacimiento)->age }} años</span>
                                @else
                            ...
                            <td>
                                <div class="d-flex flex-column gap-1" style="font-size: 0.8rem;">
                                    ...
                                    <div class="d-flex align-items-center gap-2 text-slate-500">
                                        ...
                                        <span class="text-truncate" style="max-width: 150px;">{{ $paciente->email }}</span>
                                    </div>
                                    @endif
                            ...
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end align-items-center gap-2">
                                    <!-- Ver Historial -->
                                    <a href="{{ route('pacientes.show', $paciente->id) }}" class="btn-history-soft" title="Ver Expediente">
                                        <i data-lucide="folder-open" style="width: 14px; height: 14px;"></i>
                                        Abrir
                                    </a>

                                    <!-- Editar -->
                                    <a href="{{ route('pacientes.edit', $paciente->id) }}" class="btn-action-soft btn-edit-soft" title="Editar">
                                        <i data-lucide="edit-2" style="width: 14px; height: 14px;"></i>
                                    </a>

                                    <!-- Eliminar -->
                                    <form action="{{ route('pacientes.destroy', $paciente->id) }}" method="POST" class="d-inline" data-axios-delete data-axios-no-reload="true" data-axios-remove-target="#paciente-row-{{ $paciente->id }}" data-confirm="¿Estás seguro de eliminar a {{ $paciente->nombre }}?">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="bg-light py-2 px-4 d-flex align-items-center justify-content-between flex-wrap gap-2 border-top" style="border-radius: 0 0 16px 16px;">
                <div class="text-slate-500" style="font-size: 0.8rem;">
                    Mostrando <span class="fw-bold text-dark">{{ $pacientes->firstItem() ?? 0 }}</span> - <span class="fw-bold text-dark">{{ $pacientes->lastItem() ?? 0 }}</span> de <span class="fw-bold text-dark">{{ $pacientes->total() }}</span>
                </div>
...
@endsection
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const searchForm = document.querySelector('form[action="{{ route("pacientes.index") }}"]');
            if (searchForm) {
                searchForm.addEventListener('submit', async function(e) {
                    ...
                    const fetchUrl = '{{ route("pacientes.buscar") }}?' + params.toString();
