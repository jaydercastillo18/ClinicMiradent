...
<div class="container-fluid p-0">
    ...
    <div id="view-list-wrapper" class="d-none mb-5">
        ...
        <!-- Search & Filter Bar -->
        <div class="mb-4">
            <form id="filterCitasForm" action="{{ route('citas.index') }}" method="GET" class="d-flex flex-wrap gap-2 align-items-center">
                <!-- Search Pill -->
                <div class="search-pill-inline">
                    <i data-lucide="search" style="color: #94a3b8; width: 16px; height: 16px;"></i>
                    <input type="text" name="search" placeholder="Nombre, apellido o DNI..." value="{{ request('search') }}">
                ...
                <select name="status" class="form-select bg-white shadow-sm filter-auto-submit" style="width: auto; border-radius: 100px; font-size: 0.85rem; border: 1px solid #e2e8f0; cursor: pointer; padding-left: 16px; padding-right: 32px;">
                    <option value="Todas" {{ request('status') === 'Todas' ? 'selected' : '' }}>Todos los Estados</option>
                    <option value="pendiente" {{ request('status') === 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                    <option value="confirmada" {{ request('status') === 'confirmada' ? 'selected' : '' }}>Confirmada</option>
                    <option value="en_espera" {{ request('status') === 'en_espera' ? 'selected' : '' }}>En Espera</option>
                    <option value="completada" {{ request('status') === 'completada' ? 'selected' : '' }}>Completada</option>
                    <option value="cancelada" {{ request('status') === 'cancelada' ? 'selected' : '' }}>Cancelada</option>
                </select>

                <input type="date" name="date" class="form-control bg-white shadow-sm filter-auto-submit" style="width: auto; border-radius: 100px; font-size: 0.85rem; border: 1px solid #e2e8f0; cursor: pointer; padding-left: 16px;" value="{{ request('date') }}">

                @if(request('search') || (request('status') && request('status') !== 'Todas') || request('date'))
                    <a href="{{ route('citas.index') }}" class="btn btn-light d-inline-flex align-items-center justify-content-center" style="border-radius: 100px; font-size: 0.85rem; border: 1px solid #e2e8f0;">
            ...
            @else
                            ...
                            @foreach($citas as $cita)
                                ...
                                <tr>
                                    <td class="ps-4">
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="avatar-compact">
                                                {{ strtoupper(substr($pacienteNombre, 0, 1) . substr($pacienteApellido, 0, 1)) }}
                                            </div>
                                            <div>
                                                <h6 class="mb-0" style="font-weight: 700; color: #0f172a; font-size: 0.9rem;">
                                                    {{ $pacienteNombre }} {{ $pacienteApellido }}
                                                </h6>
                                                <code style="font-size: 0.75rem; color: #64748b; background: transparent;">DNI: {{ $pacienteDni }}</code>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <span class="badge {{ $badgeBg }}" style="font-size: 0.65rem; border-radius: 6px; padding: 4px 8px; text-transform: uppercase;">
                                            {{ $cita->estado }}
                                        </span>
                                    </td>
                                    <td class="text-end pe-4">
                                        <div class="d-flex justify-content-end align-items-center gap-2">
                                            ...
                                            <button type="button" class="btn-action-soft btn-edit-soft edit-cita-btn"
                                                ...
                                                data-id="{{ $cita->id }}"
                                                data-update-url="{{ route('citas.update', ['id' => $cita->id]) }}"
                                                data-paciente-id="{{ $cita->paciente_id }}"
                                                data-paciente-nombre="{{ $pacienteNombre }} {{ $pacienteApellido }}"
                                                data-paciente-telefono="{{ $pacienteTel }}"
                                                data-doctora-id="{{ $cita->doctora_id }}"
                                                data-doctora-nombre="{{ $docNombre }}"
                                                data-servicio-id="{{ $cita->servicio_id }}"
                                                data-servicio-nombre="{{ $servicioNombre }}"
                                                data-fecha-hora="{{ str_replace(' ', 'T', $cita->fecha_hora) }}"
                                                data-motivo="{{ $cita->motivo }}"
                                                data-diagnostico="{{ $cita->diagnostico }}"
                                                data-notas="{{ $cita->notas_tratamiento }}"
                                            ...
                                            <form action="{{ route('citas.destroy', $cita->id) }}" method="POST" class="m-0 p-0" data-axios-delete data-axios-no-reload="true" data-confirm="¿Está seguro de cancelar esta cita?">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="bg-light py-2 px-4 d-flex align-items-center justify-content-between flex-wrap gap-2 border-top">
                    <div class="text-slate-500" style="font-size: 0.8rem;">
                        Mostrando <strong class="text-dark">{{ $citas->firstItem() ?? 0 }}</strong> a <strong class="text-dark">{{ $citas->lastItem() ?? 0 }}</strong> de <strong class="text-dark">{{ $citas->total() }}</strong>
                    </div>
...
<div class="modal fade" id="createCitaModal" tabindex="-1" aria-labelledby="createCitaModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px;">
            ...
            <form action="{{ route('citas.store') }}" method="POST" data-axios-submit data-axios-reset="true" data-axios-no-reload="true" data-axios-close-modal="true">
                @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <select name="doctora_id" id="create_doctora_id" class="form-select" required style="border-radius: 8px; height: 42px; border-color: var(--border-color); font-size: 0.875rem;">
                                @foreach($doctoras as $doc)
                                    <option value="{{ $doc->id }}">{{ $doc->user->name }} - {{ $doc->especialidad }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <select name="servicio_id" id="create_servicio_id" class="form-select" required style="border-radius: 8px; height: 42px; border-color: var(--border-color); font-size: 0.875rem;">
                                ...
                                @foreach($servicios as $serv)
                                    <option value="{{ $serv->id }}">{{ $serv->nombre }} (S/. {{ number_format($serv->precio, 2) }})</option>
                                @endforeach
                            </select>
                        </div>
...
<script>
    ...
    const CITAS_UPDATE_BASE_URL = '{{ url("/admin/citas") }}';
