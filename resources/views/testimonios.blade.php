...
<div class="container-fluid p-0">
    ...
    <div class="card border-0 shadow-sm mb-4" style="border-radius: 16px; border: 1px solid var(--border-color) !important;">
        <div class="card-body p-4">
            <form action="{{ route('testimonios.index') }}" method="GET" class="row g-3">
                ...
                <div class="col-12 col-sm-6 col-md-3">
                    <select name="status" class="form-select bg-white shadow-none"
                        ...
                        <option value="Todas" {{ $status === null || $status === 'Todas' ? 'selected' : '' }}>Todos los estados</option>
                        <option value="activas"   {{ $status === 'activas'   ? 'selected' : '' }}>Visibles</option>
                        <option value="inactivas" {{ $status === 'inactivas' ? 'selected' : '' }}>Ocultos</option>
                    </select>
                </div>
    ...
    @if($testimonios->isEmpty())
        <div class="card border-0 shadow-sm p-5 text-center" style="border-radius: 16px; border: 1px solid var(--border-color) !important;">
            ...
            @if($search || ($status && $status !== 'Todas'))
                <a href="{{ route('testimonios.index') }}" class="btn btn-outline-secondary px-4 mt-2" style="border-radius: 10px;">Limpiar Filtros</a>
            @endif
        </div>
    @else
        <div class="row g-3">
                    ...
                    <div class="card border-0 shadow-sm h-100" style="border-radius: 16px; border: 1px solid var(--border-color) !important;">
                        <div class="card-body p-4 d-flex flex-column gap-3">
                            ...
                            <p class="mb-0 text-muted" style="font-size: 0.9rem; line-height: 1.6; flex: 1;">
                                "{{ Str::limit($testimonio->testimonio, 140) }}"
                            </p>
                            ...
                            <div class="d-flex justify-content-between align-items-center pt-2 border-top" style="border-color: var(--border-color) !important;">
                                <div>
                                    <div style="font-weight: 700; font-size: 0.9rem; color: var(--text-main);">{{ $testimonio->nombre_paciente }}</div>
                                    @if($testimonio->fecha)
                                        <div style="font-size: 0.78rem; color: #94a3b8;">{{ $testimonio->fecha->format('d/m/Y') }}</div>
                                    @endif
                                </div>
                                <div class="d-flex gap-2">
                                    <button type="button"
                                            ...
                                            data-testimonio="{{ json_encode($testimonio) }}"
                                    ...
                                    <button type="button"
                                            ...
                                            data-nombre="{{ $testimonio->nombre_paciente }}"
                ...
                <div class="modal-body px-4 py-3 d-flex flex-column gap-3">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            ...
                            <input type="date" name="fecha" class="form-control" value="{{ date('Y-m-d') }}" style="border-radius: 10px;">
                        </div>
                    </div>
                    <div>
