...
@section('title', 'Expediente Médico')
...
<div class="record-shell">
    <section class="record-hero" aria-label="Cabecera del expediente">
        <div class="record-hero-main">
            <div class="record-avatar" aria-hidden="true">{{ $iniciales }}</div>
            <div>
                ...
                <p class="record-subtitle">Historial integral de <strong>{{ $nombreCompleto }}</strong>.</p>
            </div>
        </div>

        <div class="record-hero-actions">
            <a href="{{ route('pacientes.index') }}" class="record-btn">
                <i data-lucide="arrow-left" style="width: 16px; height: 16px;"></i>
                Volver
            </a>
            <a href="{{ route('pacientes.edit', $paciente->id) }}" class="record-btn primary">
    ...
    <section class="summary-grid" aria-label="Resumen del expediente">
        <article class="summary-card">
            ...
            <div>
                ...
                <div class="summary-note">{{ $citasCompletadas }} completada{{ $citasCompletadas === 1 ? '' : 's' }}</div>
            </div>
        </article>
        ...
        <article class="summary-card">
            ...
            <div>
                <div class="summary-value">{{ $ultimaCita?->fecha_hora?->format('d M Y') ?? 'Sin citas' }}</div>
                <div class="summary-note">{{ $ultimaCita?->servicio?->nombre ?? 'Aún no registra atención' }}</div>
            </div>
        </article>
        ...
        <article class="summary-card">
            ...
            <div>
                <div class="summary-value">S/ {{ number_format($totalPagos, 2) }}</div>
                <div class="summary-note">{{ $pagos->count() }} movimiento{{ $pagos->count() === 1 ? '' : 's' }} registrado{{ $pagos->count() === 1 ? '' : 's' }}</div>
            </div>
        </article>
        ...
        <article class="summary-card">
            ...
            <div>
                <div class="summary-value">{{ $proximaCita?->fecha_hora?->format('d M Y') ?? 'Sin programar' }}</div>
                <div class="summary-note">{{ $proximaCita?->fecha_hora?->format('h:i A') ?? 'Agenda libre para seguimiento' }}</div>
            </div>
        </article>
    </section>
    ...
    <div class="record-layout">
        <aside class="record-stack" aria-label="Datos del paciente">
                ...
                <div class="record-panel-body">
                    <div class="vitals-grid">
                        <div class="vital-item">
                            ...
                            <span class="vital-value">{{ $edad ?? '--' }}</span>
                        </div>
                        <div class="vital-item">
                            <span class="label">Sangre</span>
                            <span class="vital-value">{{ $paciente->tipo_sangre ?? 'N/R' }}</span>
                        </div>
                        <div class="vital-item">
                            <span class="label">Género</span>
                            <span class="vital-value">{{ $paciente->genero ?? '--' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="profile-list">
                        <div class="profile-item">
                            ...
                            <span>
                                ...
                                <span class="value">DNI: {{ $paciente->dni ?? 'Sin registrar' }}</span>
                            </span>
                        </div>
                        <div class="profile-item">
                            ...
                            <span>
                                ...
                                <span class="value">{{ $paciente->telefono ?? 'Sin registrar' }}</span>
                            </span>
                        </div>
                        <div class="profile-item">
                            ...
                            <span>
                                ...
                                <span class="value">{{ $paciente->email ?? 'Sin registrar' }}</span>
                            </span>
                        </div>
                        <div class="profile-item">
                            ...
                            <span>
                                ...
                                <span class="value">{{ $paciente->direccion ?? 'Sin registrar' }}</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="record-panel-body">
                    <div class="note-list">
                        <div class="note-block">
                            ...
                            <p class="note-text">{{ $paciente->antecedentes_medicos ?? 'Ninguno reportado.' }}</p>
                        </div>
                        <div class="note-block">
                            <span class="label">Medicamentos habituales</span>
                            <p class="note-text">{{ $paciente->medicamentos_habituales ?? 'Ninguno reportado.' }}</p>
                        </div>
                        <div class="note-block {{ $tieneAlergias ? 'warning' : '' }}">
                            <span class="label">Alergias</span>
                            <p class="note-text">{{ $paciente->alergias ?: 'Ninguna registrada.' }}</p>
                        </div>
                    </div>
                </div>
            </section>
                        ...
                        <div class="profile-list">
                            <div class="profile-item">
                                ...
                                <span>
                                    ...
                                    <span class="value">{{ $paciente->contacto_emergencia_nombre }}</span>
                                </span>
                            </div>
                            <div class="profile-item">
                                ...
                                <span>
                                    ...
                                    <span class="value">{{ $paciente->contacto_emergencia_parentesco ?? 'Familiar' }}</span>
                                </span>
                            </div>
                            <div class="profile-item">
                                ...
                                <span>
                                    <span class="label">Teléfono</span>
                                    <span class="value">{{ $paciente->contacto_emergencia_telefono ?? '--' }}</span>
                                </span>
                            </div>
                        </div>
                    @else
            ...
            <nav class="quick-actions" aria-label="Acciones rápidas">
                <a href="{{ route('citas.index', ['paciente_id' => $paciente->id]) }}" class="quick-action primary">
                    <i data-lucide="calendar-plus" style="width: 15px; height: 15px;"></i>
                    Cita
                </a>
                <a href="{{ route('pagos.index', ['paciente_id' => $paciente->id]) }}" class="quick-action success">
                    <i data-lucide="dollar-sign" style="width: 15px; height: 15px;"></i>
                    Pago
                </a>
                <a href="{{ route('pacientes.ficha-pdf', $paciente->id) }}" target="_blank" class="quick-action">
                ...
                <section class="tab-pane fade show active" id="pills-history" role="tabpanel" aria-labelledby="pills-history-tab">
                    <div class="record-panel content-card">
                        <div class="section-heading">
                            ...
                            <span>{{ $totalCitas }} registro{{ $totalCitas === 1 ? '' : 's' }}</span>
                        </div>
                            ...
                            <div class="timeline">
                                @foreach($citas as $cita)
                                    ...
                                    <article class="timeline-item">
                                        <span class="timeline-dot status-{{ $estadoClase }}" aria-hidden="true"></span>
                                        <div class="timeline-card">
                                            <div class="timeline-top">
                                                <div>
                                                    <div class="timeline-date">
                                                        {{ $cita->fecha_hora ? \Carbon\Carbon::parse($cita->fecha_hora)->format('d M Y') : 'Fecha pendiente' }}
                                                        @if($cita->fecha_hora)
                                                    ...
                                                    <div class="timeline-service">
                                                        ...
                                                        {{ $cita->servicio?->nombre ?? 'Tratamiento sin servicio' }}
                                                    </div>
                                                </div>
                                                <span class="status-badge status-{{ $estadoClase }}">
                                            ...
                                            <div class="detail-grid">
                                                ...
                                                @if($cita->diagnostico)
                                                    <div><strong>Diagnóstico:</strong> {{ $cita->diagnostico }}</div>
                                                @endif
                                                @if($cita->notas_tratamiento)
                                                    <div><strong>Notas:</strong> {{ $cita->notas_tratamiento }}</div>
                                                @endif
                                                @if($cita->doctora?->user?->name)
                                                    <div><strong>Atendió:</strong> {{ $cita->doctora->user->name }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    </article>
                ...
                <section class="tab-pane fade" id="pills-payments" role="tabpanel" aria-labelledby="pills-payments-tab">
                    <div class="record-panel content-card">
                        <div class="section-heading">
                            ...
                            <span>{{ $ultimoPago?->fecha_pago?->format('d/m/Y') ? 'Último pago: ' . $ultimoPago->fecha_pago->format('d/m/Y') : 'Sin pagos' }}</span>
                        </div>
                                ...
                                <table class="payments-table">
                                    ...
                                    <tbody>
                                        @foreach($pagos as $pago)
                                            ...
                                            @endphp
                                            <tr>
                                                <td data-label="Fecha">{{ $pago->fecha_pago ? \Carbon\Carbon::parse($pago->fecha_pago)->format('d/m/Y') : '--' }}</td>
                                                ...
                                                <td data-label="Tratamiento">{{ $pago->cita?->servicio?->nombre ?? 'Pago general' }}</td>
                                                <td data-label="Monto"><span class="money">S/ {{ number_format((float) $pago->monto, 2) }}</span></td>
                                                <td data-label="Estado">
                                                    <span class="status-badge status-{{ $estadoPagoClase }}">
                                                        {{ $estadoPagoLabels[$estadoPago] ?? $estadoPago }}
                                                    </span>
                                                </td>
                                            </tr>
                                        @endforeach
