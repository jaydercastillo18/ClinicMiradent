...
@section('title', 'Servicios y Tratamientos | Miradent')
...
<section class="services-catalog">
    <div class="services-container">
        ...
        <div class="services-filter-card">
            <form action="{{ route('public.servicios') }}" method="GET" class="services-search-form">
                <div class="services-search-field">
                    <i data-lucide="search" class="services-search-icon"></i>
                    <input type="text" name="search" value="{{ $search }}" placeholder="Buscar tratamiento: ortodoncia, blanqueamiento, implantes..."
            ...
            <div>
                ...
                <div class="services-filter-list">
                    @foreach($categoriaFiltros as $filter)
                        <a href="{{ $filter->url }}" class="pill {{ $filter->class ?? '' }}">
                            {{ $filter->label }}
                        </a>
                    @endforeach
                </div>
        ...
        <div class="services-grid">
            ...
            <div class="service-card fade-up" {!! 'style="animation-delay: ' . ($loop->index * 0.05) . 's;"' !!}>
                @if($servicio->image)
                <div class="service-img-wrap btn-open-modal"
                    ...
                    <span class="service-category-badge">{{ $servicio->category }}</span>
                </div>
                @else
                <div class="service-icon-fallback btn-open-modal cursor-pointer"
                    ...
                    <div class="service-icon-fallback-badge">
                        <i data-lucide="{{ $servicio->category_icon ?? 'activity' }}" class="service-icon-fallback-svg"></i>
                    </div>
                </div>
                @endif
                        ...
                        <div class="service-info-row">
                            <div>
                                ...
                                <div class="service-duration">
                                    <i data-lucide="clock"></i> {{ $servicio->duration }} min
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="service-meta-label">Precio</div>
                                <div class="price-tag">{{ $servicio->price_display }}</div>
                            </div>
                        </div>

                        <div class="service-actions">
                            <button class="btn-open-modal btn-details flex-1"
                                    data-name="{{ $servicio->name }}" data-desc="{{ $servicio->description }}"
                                    data-category="{{ $servicio->category }}" data-price="{{ $servicio->price }}"
                                    data-duration="{{ $servicio->duration }}" data-img="{{ $servicio->image }}"
                                    data-wa-url="{{ $servicio->whatsapp_url }}">
                                Detalles
                            </button>
                            <a href="{{ $servicio->whatsapp_url }}" target="_blank" class="btn-agendar flex-1">
            ...
            <div class="col-span-full py-20 bg-white rounded-[40px] border border-jade-100 shadow-sm text-center">
                ...
                <a href="{{ route('public.servicios') }}" class="inline-flex items-center gap-2 bg-jade-100 text-jade-800 px-6 py-3 rounded-full font-bold hover:bg-jade-200 transition">
                    Limpiar filtros
                </a>
            </div>
            @endforelse
        ...
        <div class="services-help">
            <div class="services-help-card">
                ...
                <div class="services-help-text">
                    ...
                    <p>{{ $doctora->trust_text }}</p>
                </div>
                <a href="{{ $doctora->consultation_url }}" target="_blank" class="services-help-button">
