...
@section('title', 'Horarios de Atención | Miradent')
...
<section class="horarios-page-header">
    <div class="container-premium text-center">
        ...
        <p>Conoce nuestra disponibilidad. La Dra. {{ $doctora->name ?: 'Miradent' }} te espera con calidez y la mejor tecnología.</p>
    </div>
</section>

<section class="horarios-container">
    <div class="container-premium">
        @if($estadoClinica['has_schedule'] ?? false)
        <div class="status-badge-container fade-up">
            <div class="status-badge hover:scale-105 transition-transform cursor-default">
                <span class="status-dot {{ $estadoClinica['open'] ? 'open' : 'closed' }}"></span>
                <span style="color: var(--jade-950);">{{ $doctora->status_label }}</span>
                <span style="color: var(--jade-300); font-weight: normal;">|</span>
                <span style="color: var(--jade-600); font-weight: 600;">{{ $doctora->status_text }}</span>
            </div>
        </div>
        @endif

        @if(!empty($doctora->horario_items))
        <div>
            @foreach($doctora->horario_items as $item)
            <div class="schedule-card fade-up {{ $item['day'] === $hoy ? 'is-today' : '' }}" {!! 'style="animation-delay: ' . ($loop->index * 0.05) . 's;"' !!}>
                ...
                <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 6px;">
                    ...
                    @if($item['has_turno2'] ?? false)
                        <div class="schedule-time" style="font-size: 1rem;">{{ $item['start2'] }} — {{ $item['end2'] }}</div>
                    @endif
                </div>
            </div>
            @endforeach
        ...
        <div class="horarios-action fade-up" style="animation-delay: 0.4s;">
            @if($estadoClinica['open'] ?? false)
                <a href="{{ route('public.reservar-cita') }}" class="btn-reservar">
